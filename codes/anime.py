...
# Função para extrair links dos episódios da página principal do anime
def extrair_links_dos_episodios(url):
    resposta = requests.get(url)
    if resposta.status_code == 200:
        soup = BeautifulSoup(resposta.text, 'html.parser')
        episodios = []
        for link in soup.find_all('a', class_='lEp epT divNumEp smallbox px-2 mx-1 text-left d-flex'):
            episodios.append(link['href'])
        return episodios
    else:
        print('Não foi possível acessar a página principal do anime.')
        return []

# Entrada do usuário para o link da página principal do anime
...
# Extraindo links dos episódios
links_episodios = extrair_links_dos_episodios(link_pagina_principal)

# Processando cada link de episódio
